...
window.addEventListener('DOMContentLoaded', function() {
  ...
  function animate() {
    drawSchematic();
    drawElectrons();
    // Automated test helper: simulate teacher lock/unlock and quiz unlock flows.
    // Call `_autoTestLockFlow()` from the browser console; it logs results and restores locks.
    window._autoTestLockFlow = function(){
      try{
        const topic = 'series';
        const el = document.querySelector('[data-topic="'+topic+'"]');
        if(!el){ console.warn('_autoTestLockFlow: topic element not found for', topic); }
        function readLocks(){ try{ return JSON.parse(localStorage.getItem('electricityLabLocks')||'{}'); }catch(e){ return {}; } }
        function writeLocks(l){ try{ localStorage.setItem('electricityLabLocks', JSON.stringify(l)); }catch(e){} }
        const original = readLocks();

        const results = { steps: [] };

            // Step 1: teacher lock (canonical: false = locked)
            writeLocks(Object.assign({}, original, { [topic]: false }));
            if(typeof applyLocksToUI === 'function') applyLocksToUI();
            const lockedNow = (el && (el.classList.contains('locked') || getComputedStyle(el).opacity < 1));
            results.steps.push({ action: 'teacherLock', ok: !!lockedNow, detail: lockedNow });

            // Step 2: teacher unlock (canonical: true = unlocked)
            writeLocks(Object.assign({}, original, { [topic]: true }));
            if(typeof applyLocksToUI === 'function') applyLocksToUI();
            const unlockedNow = (el && (!el.classList.contains('locked') && getComputedStyle(el).opacity >= 1));
            results.steps.push({ action: 'teacherUnlock', ok: !!unlockedNow, detail: unlockedNow });

            // Step 3: simulate quiz pass (student unlock) when teacher not locked
            // Ensure teacher not locking
            const before = readLocks();
            if(before[topic] === false){ results.steps.push({ action: 'skipQuizSim', ok: false, detail: 'teacher locked, skipping' }); }
            else {
              window.postMessage && window.postMessage({ type: 'QUIZ_PASSED', topic: topic }, '*');
              // read back
              const after = readLocks();
              const studentUnlocked = after[topic] === true;
              results.steps.push({ action: 'quizPass', ok: !!studentUnlocked, detail: after[topic] });
            }
        ...
        console.log('_autoTestLockFlow result:', results);
        return results;
      }catch(e){ console.error('_autoTestLockFlow error', e); return { error: String(e) }; }
    };
...
// Lock logic centralized in index.html under the single storage key 'electricityLabLocks'.
// Removed duplicate lock engine from script.js to ensure a single source-of-truth.
